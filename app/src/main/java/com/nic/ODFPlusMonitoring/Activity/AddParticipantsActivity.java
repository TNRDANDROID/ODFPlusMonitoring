...
public class AddParticipantsActivity extends AppCompatActivity implements Api.ServerResponseListener {
    ...
    private JSONObject addParticipatesJsonParams() {
        ...
        try {
            jsonObject.put(AppConstant.KEY_SERVICE_ID,"AddParticipates");
                ...
                if (childCount > 0) {
                    for (int i = 0; i < childCount; i++) {
                        ...
                        designation.getSelectedItemPosition()!=0) {
                            if (Utils.isOnline()) {
                                imageArray.put(i);
                                imageArray.put(m_mobile.getText().toString());
                                imageArray.put(m_name.getText().toString());
                                imageArray.put(designation.getSelectedItemPosition());
